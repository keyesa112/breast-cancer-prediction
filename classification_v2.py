...
def evaluasi_model(nama_model, y_test, y_pred):
    ...
    sns.heatmap(cm, annot=True, fmt='d', cmap='Blues', cbar=False, xticklabels=['Sehat (0)', 'Terdapat Kanker (1)'], yticklabels=['Sehat (0)', 'Terdapat Kanker (1)'])
...
evaluasi_df.set_index('Model').plot(kind='bar', figsize=(10, 6))
...
predictions_df.to_csv('breast-cancer-prediction.csv', index=False)
print("Hasil prediksi telah disimpan ke file 'breast-cancer-prediction.csv'.")
